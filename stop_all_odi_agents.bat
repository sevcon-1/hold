...
set ident=NTMAMSG:
set LOG=%~dp0\%~n0.log
echo %ident% %~dp0\%~n0 started at %DATE% %TIME% >> %LOG%
set /a project+=1
REM 
REM END 
REM
...
:project_1
REM NDMIS
set bin=E:\ODIHome_12_2\user_projects\domains\ndmis_odi_domain\bin
set agents=OracleDISaAgent2
GOTO AGENTS

REM 
REM Pre Prod does not have an SBCI instance
REM 
:project_2
REM SBCI
...
(for %%a in (%agents%) do (
	...
	echo %ident% %ident% Starting agent: %%a >> %LOG%
	cmd /c %bin%\startComponent.cmd %%a >> %LOG%
